...
class MicroscopeAOCompositeDevicePanel(wx.Panel):
    ...
    def OnSensorlessAO(self, event: wx.CommandEvent) -> None:
        ...
        if window is None:
            routine_objects = microAO.aoRoutines.routines[self._device.sensorless_routine]
            if routine_objects.results_viewer:
                window = routine_objects.results_viewer(None)
                self._components["sensorless_results"] = window.GetId()

        # Show the window if there is one
        if window:
            window.Show()

        # Check if a datapoint exists for this position
        datapoint_z = cockpit.interfaces.stageMover.getPosition()[2]
        if self._device.sensorless_routine.params.get("save_as_datapoint", False):
    ...
    def OnSetRoutine(self, event: wx.CommandEvent) -> None:
        ...
        routine_choices = [key.name for key in microAO.aoRoutines.routines.keys()]
        ...
        with wx.SingleChoiceDialog(
            ...
            dlg.SetSelection(
                routine_choices.index(
                    self._device.sensorless_routine.name
                )
            )
            if dlg.ShowModal() == wx.ID_OK:
                routine = list(microAO.aoRoutines.routines.keys())[dlg.GetSelection()]
                self._device.set_sensorless_routine(routine)
                logger.log.info(
                    "Set sensorless AO routine to: {}".format(routine.name)
                )

    def OnSetSensorlessParameters(self, _) -> None:
        with microAO.aoRoutines.routines[self._device.sensorless_routine].parameter_dialog(self) as dlg:
            dlg.ShowModal()
